  ...
  container.innerHTML = `
    <div class="sdash-top-bar">
      ...
      <div class="sdash-refresh-info">
        <span class="sdash-last-updated"><i class="fas fa-circle-check"></i> Updated ${now}</span>
        <a class="sdash-open-sheet-btn" href="${sheetUrl}" target="_blank" rel="noopener noreferrer">
...
function _makeGoogleChart(divId, cols, rows, isAmount) {
  ...
  let fi = 1;
  if (colA !== -1) { fmt.format(dt, fi); fi++; }
  if (colB !== -1) { fmt.format(dt, fi); }
  ...
  const options = {
    ...
    hAxis: { textStyle: { fontSize: 10, color: '#64748b' }, slantedText: true, slantedTextAngle: 38 },
    vAxis: {
...
function _buildTable(cols, rows) {
  ...
  const body = rows.map(r => {
    const cells = cols.map((c, i) => {
      ...
      if (typeof v === 'number') return `<td class="sdash-num">${isAmt ? '₹' + _fmt(v) : v}</td>`;
      return `<td>${v}</td>`;
    }).join('');
    return `<tr>${cells}</tr>`;
  }).join('');
...
export function sdashRefresh() { _load(); }
